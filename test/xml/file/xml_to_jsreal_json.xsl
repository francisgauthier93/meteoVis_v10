...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="word">
        ...
        <xsl:for-each select="key('base', base)/child::*">
            <xsl:variable name="tag" select="name(current())" />
            
            <xsl:variable name="next">
                <xsl:choose>
                    <xsl:when test="following-sibling::*[1]/name() != 'id'">
                        <xsl:value-of select="following-sibling::*[1]/name()" />
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="following-sibling::*[2]/name()" />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
                ...
                <xsl:when test="count(document('')/*/rali:ts_tag/entry[@src=$tag]) > 0">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="count(document('')/*/rali:ts_value/entry[@src=current()]) > 0">
                            <xsl:text>"</xsl:text>
                            <xsl:value-of select="document('')/*/rali:ts_value/entry[@src=current()]"/>
                            <xsl:text>"</xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>"</xsl:text>
